...
fig = plt.figure("Projectile Motion Simulation w/ Air Resistance")
...
x = np.where(t <= t_f, 2 * M * np.log(1 + C_d * rho * A * dx_dt * t / (2 * M)) / (C_d * rho * A), x_f) # Position Along The x-Axis

y_f = 2 * M * np.log(1 + C_d * rho * A * dy_dt * t_f / (2 * M)) / (C_d * rho * A) # Final Position On The y-Axis
y = np.where(t <= t_f, 2 * M * np.log(1 + C_d * rho * A * dy_dt * t / (2 * M)) / (C_d * rho * A), y_f) # Position Along The y-Axis
...
z = np.where(t <= t_f, np.where(t < t_peak, z_u, z_d), 0)  # Combines The Upward & Downward Motion


line = ax.plot(x[0], y[0], z[0], label=f'Drag Coefficient = {np.round(C_d,2)}\nInitial Velocity (x) = {np.round(dx_dt,2)}m/s\nInitial Velocity (y) = {np.round(dy_dt,2)}m/s\nInitial Velocity (z) = {np.round(dz_dt,2)}m/s\nInitial Height = {np.round(z_0,2)}m\nTime Of Impact = {np.round(t_f,2)}s\nMass = {np.round(M,2)}kg\nTerminal Velocity = {np.round(v_t,2)}m/s\n Maximum Height = {np.round(z_peak,2)}m')[0]
ax.set(xlim3d=((-10 if np.ceil(x_f * 1.25) > -10 else np.ceil(1.25 * x_f)), 10 if np.ceil(x_f * 1.25) < 10 else np.ceil(1.25 * x_f)), xlabel='X [m]')
ax.set(ylim3d=((-10 if np.ceil(y_f * 1.25) > -10 else np.ceil(1.25 * y_f)), 10 if np.ceil(y_f * 1.25) < 10 else np.ceil(1.25 * y_f)), ylabel='Y [m]')
ax.set(zlim3d=(0, 10 if np.ceil(z_peak * 1.25) < 10 else np.ceil(1.25 * z_peak)), zlabel='Z [m]')
